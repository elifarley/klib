...
fun Header.zonedDateTime(formatter: DateTimeFormatter = DateTimeFormatter.RFC_1123_DATE_TIME) = this.map({ ZonedDateTime.parse(it, formatter) }, formatter::format)
...
private fun <T> HttpHandler.performInternal(request: Request, nullWhenNotFound: Boolean = false, responseLens: (HttpMessage) -> T)
        : Pair<Status, T?> = try {
    this(request)
            .let {
                ...
                try {
                    ...
                    throw RemoteSystemProblem("Unable to parse response", request.uri.toString(), e, Status.INTERNAL_SERVER_ERROR,
    ...
    throw RemoteSystemProblem("SocketException on '${request.method}': ${e.message}", request.uri.toString(), e, Status.BAD_GATEWAY)
...
class RemoteSystemProblem : Exception {
    ...
    constructor(msg: String, remoteURI: String, cause: Throwable, localStatus: Status = Status.INTERNAL_SERVER_ERROR,
...
fun Status.withNested(nested: Status, e: Exception?) = this.description("${e?.javaClass?.simpleName?:""} (Nested: $nested)")
...
open class DetailedLensFailure(val failures: List<Failure>, override val cause: Exception? = null, val target: Any? = null)
