...
interface MockableCallResult<R> {
    ...
    fun <E> expectCall(
        result: R? = null,
        expectedInitialCallCount: Int = 0,
    ...
    operator fun <E> invoke(
        ...
        exception: Throwable? = null,
        block: (String) -> E
    ) = expectCall(result, expectedInitialCallCount, expectedCallCount, exception, block)
}
...
abstract class MockableCallHelper<R>(private val mockFilePropName: String) : MockableCall<R>(), MockableCallResult<R> {
    ...
    override fun <E> expectCall(
        result: R?,
        expectedInitialCallCount: Int,
        ...
        require(callId == null) {
            "[MOCK: $mockName] Expecting call (NESTED!) [RESULT=${
        ...
        check(expectedInitialCallCount == _mockResult.readCount) { "[MOCK: $mockName] Expected $expectedInitialCallCount as initial call count, but got ${_mockResult.readCount}" }

        if (exception != null) mockException = exception
        else mockResult = result

        block(callId).also { expectCallResult ->
            check(_mockResult.readCount == expectedInitialCallCount + expectedCallCount) { "[MOCK: $mockName] Expected call count: $expectedCallCount; actual: ${_mockResult.readCount}" }
    ...
    private fun importResultFromFile(mockFilePath: String?): R = File(mockFilePath).let { mockFile ->
        try {
            ...
            require(mockFile.canRead()) { "File '${mockFile.canonicalPath}' cannot be read." }
